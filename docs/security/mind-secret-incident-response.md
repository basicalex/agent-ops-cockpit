# Retained Context Secret Incident Response

## When to use this
Use this playbook if you suspect any retained context export, checkpoint, or repository-visible artifact may have captured credentials or other secrets.

## Immediate containment
1. Stop any active context exporters or background services that may still be ingesting or exporting unsafe content.
...
## Local cleanup
1. Remove local runtime state from the affected AOC state directory.
   - Default root: `${XDG_STATE_HOME:-$HOME/.local/state}/aoc/`
...
```bash
scripts/verify-aoc-secrets.sh
```

This checks for:
- forbidden tracked runtime DB/lock artifacts
...
2. Run the relevant repository secret/runtime safety verifier.
3. Re-run the affected export path and confirm exports are regenerated cleanly.
4. Confirm no runtime DBs or locks exist under tracked repo paths.

## Operational notes
- Live retained-context state belongs under the AOC state directory, not under the repo.
- Repo-visible context files should be treated as validated exports only.
