...
- `AOC_PULSE_OVERVIEW_ENABLED=1` (default): mission-control starts in Overview mode.
- `AOC_PULSE_OVERVIEW_ENABLED=0`: runs Work/Diff/Health-only mode.

Rollback is immediate: set `AOC_PULSE_VNEXT_ENABLED=0` and restart hub/mission-control.

## Overview Default Decision (2026-02-09)

- Decision: re-enable Overview by default after tab-scope model removed continuous
