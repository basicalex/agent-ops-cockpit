...
pub struct RuntimeTheme {
    ...
    pub ui_primary: String,
    pub ui_muted: String,
    pub ui_accent: String,
}

impl RuntimeTheme {
    pub fn project_palette(&self) -> Vec<String> {
        [
            self.cyan.clone(),
            self.magenta.clone(),
            self.orange.clone(),
...
impl ModuleConfig {
    pub fn new(config: &BTreeMap<String, String>) -> anyhow::Result<Self> {
        ...
        let disable_mouse = match config.get("disable_mouse") {
    ...
    pub fn required_event_mask(&self) -> u8 {
        self.left_parts
            .iter()
            .chain(self.center_parts.iter())
            .chain(self.right_parts.iter())
            .fold(0u8, |acc, part| acc | part.cache_mask)
    }

    pub fn needs_frame_events(&self) -> bool {
