...
assert_contains "$mind_ctx_ext" 'pi.registerCommand("mind-resume", {'
...
assert_contains "$mind_lib" 'export async function ingestMindMessage(message: any, ctx: ExtensionContext): Promise<{ ok: boolean; error?: string; fallbackUsed?: boolean }> {'
assert_contains "$mind_lib" 'export async function sendMindCompactionCheckpoint(event: any, ctx: ExtensionContext): Promise<{ ok: boolean; error?: string; fallbackUsed?: boolean }> {'
