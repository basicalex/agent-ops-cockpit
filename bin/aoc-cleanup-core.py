...
def zellij_action_json(session, *action_args):
    ...
    if completed.returncode != 0:
        ...
        if (
            "wasn't recognized" in stderr
            or "found argument" in stderr
            or "isn't valid in this context" in stderr
        ):
            return None
        return None
...
def collect_agent_tokens(value):
    tokens = []
    if isinstance(value, str):
        ...
        for item in value.values():
            tokens.extend(collect_agent_tokens(item))
    return tokens


def parse_agent_id_from_name(name):
    if not name:
        return ""
    stripped = name.strip()
    if stripped.startswith("Agent [") and "]" in stripped:
        return stripped.split("[", 1)[1].split("]", 1)[0].strip()
    if stripped.startswith("aoc:"):
        return stripped.split(":", 1)[1].strip()
    return ""


def is_agentish_pane_name(name):
    if not name:
        return False
    stripped = name.strip()
    return (
...
def parse_native_inventory(session, projects_base=None):
    tabs_value = zellij_action_json(session, "action", "list-tabs", "--json")
    panes_value = zellij_action_json(session, "action", "list-panes", "--json")
    if panes_value is None or tabs_value is None:
        return None

    tabs = json_items(tabs_value, "tabs", "data", "items")
    panes = json_items(panes_value, "panes", "data", "items")
    tab_index_by_name = {}
    for idx, tab in enumerate(tabs, start=1):
        if not isinstance(tab, dict):
            continue
        index = first_u64(tab, "position", "index", "tab_index", "tabPosition")
        if index is None:
            index = idx
        name = first_string(tab, "name", "tab_name", "title")
        if name:
            tab_index_by_name[name] = index
    ...
    for pane in panes:
        if not isinstance(pane, dict):
            continue
        pane_id = first_string(pane, "id", "pane_id", "paneId")
        if pane_id:
            pane_ids.add(pane_id)
        pane_name = first_string(pane, "name", "pane_name", "paneName", "title")
        tab_name = first_string(pane, "tab_name", "tabName")
        tab_index = first_u64(pane, "tab_position", "tab_index", "position", "tabPosition")
        if tab_index is None and tab_name:
            tab_index = tab_index_by_name.get(tab_name)
        if tab_index is None:
            tab_index = 0
        tab_key = (tab_index, tab_name)
        meta = tabs_meta.setdefault(tab_key, {"cwds": [], "agent_count": 0})

        agent_id = parse_agent_id_from_name(pane_name)
        is_agent_named = is_agentish_pane_name(pane_name)
        if agent_id:
        ...
        token_fields = [
            ...
            pane.get("argv"),
            pane.get("args"),
            pane.get("command_line"),
            pane.get("pane_title"),
        ]
        pane_tokens = []
        for field in token_fields:
            pane_tokens.extend(collect_agent_tokens(field))
        ...
        if cwd and (is_agent_named or any(AGENT_NAME_PATTERN.search(token) for token in pane_tokens)):
            agent_cwds.add(cwd)
            meta["cwds"].append(cwd)

    for meta in tabs_meta.values():
        if not meta["cwds"] or meta["agent_count"] <= 0:
            continue
        root = collections.Counter(meta["cwds"]).most_common(1)[0][0]
        expected_cwds[root] += meta["agent_count"]
    ...
    return {
        ...
        "agent_cwds": agent_cwds,
        "expected_cwds": expected_cwds,
