...
impl HubState {
	...
	async fn snapshot_state_messages(&self) -> Vec<Vec<u8>> {
		...
		for id in ids {
			if let Some(entry) = state.get(&id) {
				...
				if let Some(payload) = entry.task_summary.as_ref() {
					messages.push(payload.clone());
				}
			}
		}
	...
	async fn update_state(&self, agent_id: &str, kind: &str, raw: &[u8]) {
		...
		let entry = state.entry(agent_id.to_string()).or_insert_with(|| AgentState {
		...
		match kind {
			...
			"task_summary" => entry.task_summary = Some(raw.to_vec()),
			_ => {}
		}
	...
	fn start_stale_reaper(self: Arc<Self>) {
		...
		tokio::spawn(async move {
			...
			loop {
				ticker.tick().await;
				let clients = hub.clients.read().await.values().cloned().collect::<Vec<_>>();
		...
		match msg.r#type.as_str() {
			"agent_status" => {
				let payload = match parse_agent_status(&msg.payload) {
					Ok(value) => value,
					Err(err) => {
						self.send_error(client, "invalid_payload", err, msg.request_id.as_deref()).await;
						warn!(event = "payload_invalid", error = err);
						return;
					}
				};
				if client.role != "publisher" || (!client.agent_id.is_empty() && payload.agent_id != client.agent_id) {
					self.send_error(client, "agent_id_mismatch", "agent id mismatch", msg.request_id.as_deref()).await;
					warn!(event = "agent_id_mismatch", conn_id = %client.conn_id);
					return;
				}
				self.update_state(&payload.agent_id, "agent_status", raw).await;
			...
			"diff_summary" => {
				...
				self.update_state(&payload.agent_id, "diff_summary", raw).await;
				self.broadcast(raw).await;
			}
			"task_summary" => {
				...
				self.update_state(&payload.agent_id, "task_summary", raw).await;
				self.broadcast(raw).await;
			}
			"task_update" => {
			...
			"diff_patch_request" => {
				...
				if client.role != "subscriber" {
					self.send_error(client, "role_violation", "subscriber role required", msg.request_id.as_deref()).await;
			...
			"diff_patch_response" => {
				...
				if let Some(patch) = &payload.patch {
					if patch.len() > MAX_PATCH_BYTES {
						self.send_error(client, "patch_too_large", "patch exceeds limit", msg.request_id.as_deref()).await;
			...
			"error" => {
				if parse_error(&msg.payload).is_err() {
					self.send_error(client, "invalid_payload", "invalid error payload", msg.request_id.as_deref()).await;
			...
			"hello" => {
				self.send_error(client, "unexpected_hello", "unexpected hello", msg.request_id.as_deref()).await;
				warn!(event = "unexpected_hello", conn_id = %client.conn_id);
			}
			other => {
				self.send_error(client, "unknown_message", "unknown message type", msg.request_id.as_deref()).await;
				warn!(event = "unknown_message", conn_id = %client.conn_id, r#type = other);
			}
		}
	}

	async fn send_error(&self, client: &Client, code: &str, message: &str, request_id: Option<&str>) {
...
fn parse_hello(payload: &Value) -> Result<HelloPayload, &'static str> {
	let value: HelloPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
...
fn parse_agent_status(payload: &Value) -> Result<AgentStatusPayload, &'static str> {
	let value: AgentStatusPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
	if value.agent_id.is_empty() || value.status.is_empty() || value.pane_id.is_empty() || value.project_root.is_empty() {
		return Err("missing_fields");
	}
	Ok(value)
}
...
fn parse_diff_summary(payload: &Value) -> Result<DiffSummaryPayload, &'static str> {
	let value: DiffSummaryPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
...
fn parse_diff_patch_request(payload: &Value) -> Result<DiffPatchRequestPayload, &'static str> {
	let value: DiffPatchRequestPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
...
fn parse_diff_patch_response(payload: &Value) -> Result<DiffPatchResponsePayload, &'static str> {
	let value: DiffPatchResponsePayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
...
fn parse_task_summary(payload: &Value) -> Result<TaskSummaryPayload, &'static str> {
	let value: TaskSummaryPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
...
fn parse_task_update(payload: &Value) -> Result<TaskUpdatePayload, &'static str> {
	let value: TaskUpdatePayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
	if value.agent_id.is_empty() || value.tag.is_empty() || value.action.is_empty() || value.task.is_null() {
		return Err("missing_fields");
	}
	Ok(value)
}
...
fn parse_heartbeat(payload: &Value) -> Result<HeartbeatPayload, &'static str> {
	let value: HeartbeatPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
	if value.agent_id.is_empty() || value.pid == 0 || value.cwd.is_empty() || value.last_update.is_empty() {
		return Err("missing_fields");
	}
	Ok(value)
}
...
fn parse_error(payload: &Value) -> Result<ErrorPayload, &'static str> {
	let value: ErrorPayload = serde_json::from_value(payload.clone()).map_err(|_| "invalid_payload")?;
