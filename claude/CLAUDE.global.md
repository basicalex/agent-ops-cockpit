...
- **Artifacts and pages.** Any artifact, page, report, brief, board, dashboard, or HTML deliverable the user asks to see or share goes through the `/artifact-pages` skill (`~/.claude/skills/artifact-pages/`): the page body lives in `~/dev/artifact-library/<project>/<slug>.html`, `artifact-pages add` + `artifact-pages publish` put it at `https://docs.intrface.eu/<project>/<slug>/` (private; owner login), and `artifact-pages share` or `visibility public` open it to a client. "Give me an artifact" or "report this as an artifact" means this docs page, nothing else. Do not use the claude.ai Artifact tool unless the user explicitly asks for a claude.ai artifact: pages there are bound to one Claude account and cannot be shared privately across accounts. Pages are `research` by default; `decision` (briefs, specs, plans, charters the team builds against) is set only by the user via `artifact-pages decide`, an agent may suggest it but never sets it unprompted. Reading is local and one command: `artifact-pages decisions <project>` lists the active decisions, `artifact-pages show <project>/<slug>` prints a page as text, `artifact-pages find <words>` searches every page; never fetch docs.intrface.eu or open a claude.ai artifact for content. Before proposing direction in a project, run `artifact-pages decisions <project>` and read what it lists. When the user says they left comments on a page, run `artifact-pages comments <project>`, act on each open thread, wrap the changed passages in `<ins class="rev" data-comment="<id>">`, publish, then `artifact-pages reply <id> "<what changed>" --addressed`; never mark a thread accepted. This holds in every harness (claude, claude-codex) and every project.
