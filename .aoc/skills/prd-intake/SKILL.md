...
4. Apply task changes with `aoc-task` primitives:
   ...
   - Link task PRD: `aoc-task prd set <id> <path> --tag <tag>`
   - Manage dependencies/status/subtasks with `aoc-task edit --depends`, `aoc-task status`, `aoc-task sub ...`
5. For explicit replace requests, remove target-tag tasks first using `aoc-task rm <id> --tag <tag>` then recreate.
