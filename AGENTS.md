...
*   **Nature:** Ephemeral, Auto-generated.
*   **Content:** File tree, `README.md` snapshot, basic operational rules.
*   **Agent Action:** **Read Only.** Do not edit this file manually; it is overwritten by `aoc-init`. Use it to understand *where* you are and *what* the project structure looks like.
...
1.  **Orient:** Run `aoc-init` (if needed) or check `.gemini/GEMINI.md` to see the files involved.
...
*   `aoc-init`: Bootstraps the `.gemini` and `.taskmaster` folders.
